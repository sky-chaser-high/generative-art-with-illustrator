...
function main() {
    ...
    for (var j = 0; j < count; j++) {
        var d = artboard.width / count;
        var x = d * j + d / 2;
        var num = Math.ceil(Math.random() * 10);
        var steps = size / num;
        var s = Math.round(Math.random() * steps);
        var color = {
        ...
        for (var i = 0; i < num; i++) {
            (color.r + s < 255) ? color.r += s : color.r = 255;
            (color.g + s < 255) ? color.g += s : color.g = 255;
            (color.b + s < 255) ? color.b += s : color.b = 255;
            createEllipse(
                aw,
                x,
                artboard.center.y * -1,
