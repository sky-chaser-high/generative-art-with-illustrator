...
function main() {
    ...
    for (var i = 0; i < frame; i++) {
        var line = randomChord(aw, artboard.center, size, color);
        line.opacity = Math.random() * 100;
    }
...
function randomChord(layer, pos, size, color) {
    var angle1 = map(Math.random(), 0, 1, 0, Math.PI * 2);
    var xpos1 = size + size * Math.cos(angle1);
    var ypos1 = size + size * Math.sin(angle1);

    var angle2 = map(Math.random(), 0, 1, 0, Math.PI * 2);
    var xpos2 = size + size * Math.cos(angle2);
    var ypos2 = size + size * Math.sin(angle2);
    ...
    var points = [
        [center.x + xpos1, center.y * -1 + ypos1 * -1],
        [center.x + xpos2, center.y * -1 + ypos2 * -1]
    ];

    return createLine(layer, points, color);
}
