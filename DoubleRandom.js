...
function main() {
    ...
    for (var x = 0; x < steps; x++) {
        var d = diameter * Math.random();
        color.blue = Math.random() * 255;
        createEllipse(
            aw,
            artboard.width / steps * x + diameter / 2,
            artboard.center.y * -1 + map(Math.random(), 0, 1, -rand, rand),
            d,
            d,
            color
        );
        rand += map(Math.random(), 0, 1, -x / 10, x / 10);
    }
