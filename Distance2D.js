...
function main() {
    ...
    if (!existsLayer(bg)) {
        createLayer(bg);
        createBackground(bg, artboard.width, artboard.height, setRGBColor(222, 224, 228));
    }
    ...
    var noiseScale = 0.03;
    var diameter = 68;
    var xpos = artboard.center.x;
    var ypos = artboard.center.y;
    var maxDistance = dist(0, 0, artboard.width, artboard.height);
    var range1 = { min: 14, max: 15 };
    var range2 = { min: 6, max: 7 };
    var maxColorValue = 255;
    var color = setRGBColor(maxColorValue);

    for (var x = 0; x <= artboard.width; x += 20) {
        for (var y = 0; y <= artboard.height; y += 20) {
            var size = dist(xpos, ypos, x, y);
            size = (size / maxDistance) * diameter;

            var n = noise(x + noiseScale, y + noiseScale) * size * 6;

            if (range1.min < size + n && size + n < range1.max) {
                color.red = Math.random() * maxColorValue;
                color.green = Math.random() * maxColorValue;
            }
            else if (range2.min < size + n && size + n < range2.max) {
                color.green = Math.random() * maxColorValue;
                color.blue = Math.random() * maxColorValue;
            }

            createEllipse(aw, x, y * -1, size + n, size + n, color);
